...
public class SetMinYCommand implements ArenaSubCommand {

    private final GradientPillars plugin;

    public SetMinYCommand(GradientPillars plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean execute(Player player, String[] args) {
        if (args.length < 2) {
            player.sendMessage(plugin.getMessageManager().getPrefixedComponent(
                    "commands.usage", "usage", "/gp arena miny <arena> <y>"));
            return true;
        }
        ...
        try {
            ...
            player.sendMessage(plugin.getMessageManager().getPrefixedComponent(
                    "arena.setup.min-y-set", "y", String.valueOf(minY)));
    ...
    public String getDescription() {
        return "Встановити мінімальну висоту (поріг смерті)";
    }

    @Override
    public String getUsage() {
        return "/gp arena miny <arena> <y>";
    }
}
